...
<template>
  <div class="rounded-lg min-w-10 w-[35rem] max-w-[40rem]">
    <button
      type="button"
      :class="{
    ...
    >
      ...
      <img
        src="../../../media/img/downArrow.png"
        class="flex size-5 filter invert ml-auto hover:animate-spin duration-200"
        alt="Flecha"
      />
    </button>
...
.active {
  @apply opacity-100 max-h-fit min-w-10 -mt-1 py-2 z-0 bg-white rounded-b-md shadow-md shadow-slate-800;
  transform: scaleY(1);
}

.custom-button {
        @apply flex items-center justify-center bg-gradient-to-l from-orange-400 to-orange-600 shadow-md text-slate-100 p-2 rounded-lg w-full z-10;
      }
</style>